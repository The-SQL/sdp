...
const mockSupabaseCourse: SupabaseCourseList = {
  ...
  course_feedback: [{ rating: 5 }, { rating: 3 }], // avg 4
  user_courses: [{ id: "1" }, { id: "2" }], // 2 students
...
describe("server db functions", () => {
  ...
  describe("insertUser", () => {
    it("inserts a user, calls initializers, and returns data", async () => {
      ...
      (createServerClient as jest.Mock).mockResolvedValue(mockClient); // Act
    ...
    it("throws and does not call initializers when insert fails", async () => {
      ...
      (createServerClient as jest.Mock).mockResolvedValue(mockClient); // Act + Assert
  ...
  describe("checkUserExists", () => {
    it("returns true when user exists", async () => {
      ...
      const exists = await checkUserExists("c1"); // Assert
    ...
    it("returns false when no rows match", async () => {
      ...
      const exists = await checkUserExists("c2"); // Assert
  ...
  describe("getCoursesByAuthor", () => {
    ...
    it("returns transformed authored courses", async () => {
      createGetCoursesMock([mockSupabaseCourse], [], [], {});
      const result = await getCoursesByAuthor("author1");
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe("course1");
      expect(result[0].rating).toBe(4);
      expect(result[0].students).toBe(2);
      expect(result[0].reviews).toBe(2);
      expect(result[0].tags).toEqual(["React", "TS"]);
      expect(result[0].isCollaborator).toBe(false);
    });
    ...
    it("returns transformed collaborator courses", async () => {
      ...
      createGetCoursesMock([], [{ course_id: "course2" }], [collabCourse], {});
      const result = await getCoursesByAuthor("author1");
      expect(result).toHaveLength(1);
  ...
  describe("applyCourseChangeRequest", () => {
    ...
    const mockCourseData = { author_id: "author1" }; // Helper to create the full procedural mock

    const createApplyMock = (request: any, course: any, errors: any = {}) => {
      const mockClient = {
        from: (table: string) => {
    ...
    it("throws if request is not found", async () => {
      ...
      await expect(applyCourseChangeRequest("req1", "author1")).rejects.toThrow(
        "not found"
      );
    });
    ...
    it("throws if approver is not the author", async () => {
      ...
      await expect(applyCourseChangeRequest("req1", "author1")).rejects.toThrow(
        "Only the course author can approve change requests"
      );
    });
