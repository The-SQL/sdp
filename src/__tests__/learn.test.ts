...
import { CourseWithContent, Lesson, UnitWithLessons } from "@/utils/types";
...
describe("learn db functions", () => {
  ...
  describe("getCourseWithContent", () => {
    it("returns a course and sorts its units and lessons", async () => {
      ...
      const result = await getCourseWithContent("course1"); // Assert

      expect(result).not.toBeNull(); // Check that units are sorted
      expect(result?.units[0].id).toBe("unit2");
      expect(result?.units[1].id).toBe("unit1"); // Check that lessons within the unit are sorted
